<script>
  ...
  const selectOption = (e) => {
    ...
    searchForOne('input[form="new-user-form"][name="role"]').value =
  ...
  if (userRole === 'admin') {
    deleteUser = async (e) => {
      ...
      const response = await fetch('/admin/users', request).then((r) =>
        r.json()
      )
...
<div class="create-user">
  <form on:submit={createUser} id="new-user-form" style="display: none;" />
  <h2>Create User</h2>
  <p>
    Provide the credentials for the new user you wish to create. Password and
    username will automatically be assigned, and the user will receive these
  ...
  <div class="new-user-info">
    ...
    <input
      ...
      style="display: none;"
    />
    <div tabindex="0" on:click={showRoleOptions} class="select-role-wrapper">
      <p>Select a role</p>
